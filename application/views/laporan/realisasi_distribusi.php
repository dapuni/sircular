<fieldset>
	<legend>Laporan Data Realisasi Distribusi</legend>
	<form action="<?php echo base_url()?>laporan/realisasi_distribusi" method="get">
					...
					if ($laporanx['agent_id']  == $detail_agent['agent_id'] && $agent_id != $laporanx['agent_id']) 
					{
						...
						echo "<tr>";
							...
							foreach ($prev_laporan as $prev_laporanx) 
							{
								...
								{
									$prev_quota = $prev_quota + $prev_laporanx['quota'] + $prev_laporanx['consigned'];
								}
							}
...
<script type="text/javascript">
	$(function(){
		...
		{
		    document.location.href ="<?php echo base_url();?>laporan/realisasi_distribusi?majalah="+$('#majalah').val();
		});
		//autoselect
		...
		$('#cetak').click(function(){
			window.open("<?php echo base_url();?>laporan/realisasi_distribusi/cetak?majalah="+$('#majalah').val()+"&edisi="+$('#edisi').val()+"&prev_edisi="+$('#prev_edisi').val(), "", "width=400, height=500");
		});

		$('#pdf').click(function(){
			window.open("<?php echo base_url();?>laporan/realisasi_distribusi/pdf?majalah="+$('#majalah').val()+"&edisi="+$('#edisi').val()+"&prev_edisi="+$('#prev_edisi').val(), "");
		});

		$('#excel').click(function(){
			document.location.href ="<?php echo base_url();?>laporan/realisasi_distribusi/excel?majalah="+$('#majalah').val()+"&edisi="+$('#edisi').val()+"&prev_edisi="+$('#prev_edisi').val();
		});
	});
</script>