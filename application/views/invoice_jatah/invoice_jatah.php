...
?>
<fieldset>
	<legend>Invoice Jatah</legend>
	<form action="<?php echo base_url()?>tagihan/invoice_jatah" method="get">
		<div class="col-xs-2">
		...
		<div class="col-xs-2">
			...
			<a href="tagihan/invoice_jatah/add" class="btn btn-default">Tambah</a>
		</div>
		<div class="col-xs-2">
		...
		<tbody>
			<?php
				...
				{ ?>
					<tr>
						<td><?php echo $no++ ?></td>
						<td><?php echo date('y',strtotime($keyinvoice['date_created'])).'/'.date('m',strtotime($keyinvoice['date_created'])).'/'.$keyinvoice['nama_majalah'].'/'.$keyinvoice['no_invoice'] ?></td>
						...
						<td style="text-align:center" ><a href="tagihan/invoice_jatah/detail/<?php echo $keyinvoice['invoice_id'] ?>" class="btn btn-primary btn-sm" >Detail</a> <a class="btn btn-primary btn-sm" onclick="cetak(<?php echo $keyinvoice['invoice_id'] ?>)" ><span class="glyphicon glyphicon-print"></span> Invoice</a></td>
					</tr>
				<?php }
			?>
		</tbody>
...
<script type="text/javascript">
	...
	function cetak(id){
		...
		window.open("<?php echo base_url();?>tagihan/invoice_jatah/cetak?id="+id+"&pajak="+pajak, "", "width=400, height=500");
	};
</script>